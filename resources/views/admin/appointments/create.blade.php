...
@section('scripts')
    <script>
        $(document).ready(function () {
            $('body').on('change', 'select#doctor_id', function () {
                const id = $(this).val()
                if (id){
                    populateServices(id);
                }else{
            ...
            if ($('select#doctor_id').val()){
                populateServices($('select#doctor_id').val());
            }
        });

        const populateServices = (id) =>{
            $.ajax({
                url: `{{route('admin.appointments.services')}}`,
                data: {id},
                success: data => {
                    console.log(data)
                    for (const id in data){
                        $('#service_id').append(`<option value="${id}">${data[id]}</option>`)
                    }
                }
            })
        }
